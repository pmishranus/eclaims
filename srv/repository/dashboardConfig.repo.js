...
module.exports = {
    fetchHeaderList: async function (fieldType, processCode) {
        ...
        let fetchHeaderList = await cds.run(SELECT.from("NUSEXT_UTILITY_DASHBOARD_CONFIG").where(queryParameter));
    ...
    fetchHeaderDetails: async function (refKey, processCode) {
        ...
        let fetchHeaderDetails = await cds.run(
            SELECT.from("NUSEXT_UTILITY_DASHBOARD_CONFIG").where(queryParameter).orderBy("SEQUENCE")
        );
    ...
    fetchClaimCode: async function (refKey, processCode, staffId) {
        ...
        let fetchCostDist = await cds.run(
            SELECT.from(" NUSEXT_UTILITY_DASHBOARD_CONFIG  as dc ")
