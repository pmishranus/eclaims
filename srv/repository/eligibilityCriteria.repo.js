...
async function fetchClaimTypes(staffId) {
    ...
    let fetchClaimTypes = await cds.run (SELECT
        .columns(
        ...
        .join('NUSEXT_MASTER_DATA_MASTER_CLAIM_TYPE as mc').on('ec.CLAIM_TYPE = mc.CLAIM_TYPE_C')
        .join('NUSEXT_MASTER_DATA_CHRS_JOB_INFO as cj1').on('cj1.SF_STF_NUMBER = ec.SF_STF_NUMBER and cj1.END_DATE >= ec.END_DATE')
        .where(
       ...
       and ec.CLAIM_TYPE in (?, ?, ?)`,
            staffId, staffId, staffId, ...claimTypes
        )
        .groupBy(
            'ec.CLAIM_TYPE',
            'mc.CLAIM_TYPE_T',
    ...
    const fetchClaimTypesForCw = await cds.run(
        SELECT
          ...
          .where([
            ...
            { ref: ['cj1', 'SF_STF_NUMBER'], in:
              SELECT.from('NUSEXT_MASTER_DATA_CHRS_JOB_INFO as cj2')
                    .columns(['SF_STF_NUMBER'])
                    .where([
                      { func: 'upper', args: [{ ref: ['cj2', 'NUSNET_ID'] }] }, '=', staffId.toUpperCase(),
                      'or',
                      { ref: ['cj2', 'SF_STF_NUMBER'] }, '=', staffId,
                      'or',
                      { ref: ['cj2', 'STF_NUMBER'] }, '=', staffId
                    ])
            },
            // current date constraints
            { ref: ['ec', 'START_DATE'] }, '<=', { val: new Date() },
            { ref: ['ec', 'SUBMISSION_END_DATE'] }, '>=', { val: new Date() },
            // period overlap
            { ref: ['ec', 'START_DATE'] }, '<=', { ref: ['cj1', 'END_DATE'] },
            { ref: ['ec', 'END_DATE'] }, '>=', { ref: ['cj1', 'START_DATE'] },
            // claim type
            { ref: ['ec', 'CLAIM_TYPE'] }, '=', '102'
          ])
          .groupBy(
            'ec.CLAIM_TYPE',
            'mc.CLAIM_TYPE_T',
...
module.exports = {
    fetchWorkingHours, fetchClaimTypes,fetchClaimTypesForCw
}